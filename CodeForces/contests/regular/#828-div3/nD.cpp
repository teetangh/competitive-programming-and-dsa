...
int main()
{
    ...
    while (tc--)
    {
        ...
        ulli target = 1 << n;
        debug(target);
        ulli factorsOf2inTarget = 0;
        ulli factorsOf2inProd = 0;
        while (target % 2 == 0)
        {
            factorsOf2inTarget++;
            target /= 2;
        ...
        debug(factorsOf2inTarget);
        debug(factorsOf2inProd);
        if (factorsOf2inProd >= factorsOf2inTarget)
        {
            cout << 0 << endl;
        }
        else
        {
            int diff = factorsOf2inTarget - factorsOf2inProd;
            int count = 0;
            priority_queue<int> powersOf2;
            for (int i = n; i >= 1; i--)
            {
                auto temp = i;
                int power = 0;
            ...
            while (!powersOf2.empty())
            {
                int power = powersOf2.top();
                powersOf2.pop();
                if (diff >= power)
                {
                    diff -= power;
