...
// Explanation:
// Testcase 1: (1, 4), (2, 2), (2, 3), (2, 4), (2, 5), (3, 2), (3, 4), (4, 2), (4, 3), (4, 4) are the spiritual cities which geek will visit.#include <iostream>
#include <iomanip>
#include <bits/stdc++.h>
...
int maxCities(vector<vector<char>> &a, int n, int m)
{
    ...
    vector<vector<bool>> spiritual_matrix;
    spiritual_matrix.resize(n, vector<bool>(a[0].size(), false));
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            if (a[i][j] == '*')
                spiritual_matrix[i][j] = false; // legend.emplace_back(spiritual(i, j, false));
            ...
            else
            {
                ...
                spiritual_matrix[i][j] = spirituality; // legend.emplace_back(spiritual(i, j, spirituality));
            }
        }
    }
    ...
    for (int i = 0; i < spiritual_matrix.size(); i++)
    {
        ...
        {
            cout << spiritual_matrix[i][j] << " ";
            sum += spiritual_matrix[i][j];
        }
        cout << endl;
    }
