...
bool solveKnightMove(int board[D][D], int n, int move_no, int curRow, int curCol)
{
    ...
    for (int curDir = 0; curDir < 8; ++curDir)
    {
        ...
        if (canPlace(board, n, nextRow, nextCol) == true)
        {
            ...
            /* COMMENTING THIS TO CHECK OTHER POSSIBLITIES
...
{
#ifndef ONLINE_JUDGE
    freopen("knight_tour_multiple_solutions_input.txt", "r", stdin);
    freopen("knight_tour_multiple_solutions_output.txt", "w", stdout);
#endif
