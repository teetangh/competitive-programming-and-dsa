...
int main()
{
    ...
    while (tc--)
    {
        lli size;
        cin >> size;
        vector<lli> ground(size);
        vector<lli> water(size);
        vector<lli> strength(size); // sum of beating water and ground
        ...
        for (lli i = 0; i < size; i++)
        {
            ground_set.insert(ground[i]);
            water_set.insert(water[i]);
        }

        for (lli i = 0; i < size; i++)
        {
            lli ground_index = ground_set.order_of_key(ground[i]);
            lli water_index = water_set.order_of_key(water[i]);
            strength[i] = ground_index + water_index;
        }
        // debug(ground);
        // debug(water);
        // debug(strength);

        lli max_strength = *max_element(strength.begin(), strength.end());
        lli count_max = count(strength.begin(), strength.end(), max_strength);
        cout << count_max << endl;
    }
