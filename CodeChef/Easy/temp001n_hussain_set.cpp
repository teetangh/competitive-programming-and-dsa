...
int query_answer(vector<int> arr, queue<int> my_queue, int counter, int current_query)
{
    ...
    while (counter < current_query && (!arr.empty() || !my_queue.empty()))
    {
        ...
        if (arr.empty())
        {
            current_query_answer = my_queue.front();
            my_queue.pop();
        }

        if (!arr.empty() && (my_queue.empty() || arr.back() >= my_queue.front()))
        {
            current_query_answer = arr.back();
            arr.pop_back();
        }
        else if (!arr.empty() && (my_queue.empty() || arr.back() < my_queue.front()))
        {
            current_query_answer = my_queue.front();
            my_queue.pop();
        }
        if (current_query_answer / 2 > 0)
            my_queue.push(current_query_answer / 2);
    }
